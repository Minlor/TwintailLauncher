...
export default function LauncherSettings({ setOpenPopup, settings, fetchSettings }: LauncherSettingsProps) {
    ...
    const updateSetting = async (key: string, value: any) => {
        // Optimistic update if needed, but usually we rely on fetchSettings after invoke
        // However, standard pattern in this app seems to be component-level fetching after change
        // We can replicate the old CheckBox/Input behavior:
        // invoke(endpoint, { ... }).then(() => fetchSettings())

        // For simple key-value updates, we might need specific endpoints or a generic one if available.
        // Looking at old code: CheckBox uses `update_settings_{id}` usually or generic?
        // Wait, the old CheckBox component took `fetchSettings` and `id`. 
        // Let's assume specific invokes based on ID for now, or use a generic update if I find one.
        // Actually the old code used specific endpoints for most things.
        // "third_party_repo_updates" -> invoke based on ID?

        // Let's verify the CheckBox implementation or assume specific endpoints.
        // Re-reading SettingsGlobal.tsx... it passes `id` to CheckBox.
        // CheckBox implementation likely does `invoke("update_settings_" + id, { enabled: value })`.
        ...
        try {
            if (typeof value === "boolean") {
                ...
                // Special cases based on ID mapping from old code:
                if (key === "download_speed_limit") {
                    // Command has _cmd suffix and uses speedLimit parameter
    ...
    const handleTabChange = (newTabId: string) => {
        ...
        // If moving down the list (higher index), slide UP (content enters from bottom)
        // If moving up the list (lower index), slide DOWN (content enters from top)
    ...
    return (
        <SettingsLayout title="Launcher Settings" onClose={() => setOpenPopup(POPUPS.NONE)}>
                ...
                >
                    ...
                    {activeTab === "about" && (
                        <div className="flex flex-col items-center justify-center h-full text-center p-8 relative overflow-hidden">
                            ...
                            />
                            {/* Fallback Icon if image missing (displayed via CSS if needed, but here we just render it conditionally or alongside) */}
                            <div className="bg-gradient-to-br from-pink-500 to-violet-600 p-4 rounded-2xl mb-6 shadow-[0_0_30px_rgba(236,72,153,0.3)] hidden">
