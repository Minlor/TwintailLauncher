...
use crate::utils::{
    AddInstallRsp, DownloadSizesRsp, PathResolve, ResumeStatesRsp, apply_xxmi_tweaks, copy_dir_all,
    download_or_update_fps_unlock, download_or_update_jadeite, download_or_update_xxmi,
...
#[cfg(target_os = "linux")]
use crate::utils::{
    is_flatpak, patch_aki, run_async_command, runner_from_runner_version,
        ...
        || game_background.is_empty()
    {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let dxvk = Path::new(gs.default_dxvk_path.as_str())
                .follow_symlink()
                .unwrap();
            let prefix_loc = Path::new(&runner_prefix)
                .join(cuid.clone())
                .follow_symlink()
                .unwrap();
            ...
            if gm.biz == "wuwa_global" && skip_game_dl {
                let target = Path::new(&directory.clone())
            ...
            if gm.biz == "bh3_global" {
                use_jadeite = true;
                let jadeite = Path::new(&gs.jadeite_path)
                    .follow_symlink()
                    .unwrap()
                    .to_path_buf();
...
pub fn update_install_use_jadeite(app: AppHandle, id: String, enabled: bool) -> Option<bool> {
    ...
    if manifest.is_some() {
        let m = manifest.unwrap();
        let p = Path::new(&settings.jadeite_path)
            .follow_symlink()
            .unwrap()
            .to_path_buf();
        ...
        if enabled {
            download_or_update_jadeite(p, false);
        }
        Some(true)
    } else {
        None
...
pub fn update_install_use_xxmi(app: AppHandle, id: String, enabled: bool) -> Option<bool> {
    ...
    if manifest.is_some() {
        let m = manifest.unwrap();
        let p = Path::new(&settings.xxmi_path)
            .follow_symlink()
            .unwrap()
            .to_path_buf();
...
pub fn update_install_use_fps_unlock(app: AppHandle, id: String, enabled: bool) -> Option<bool> {
    ...
    if manifest.is_some() {
        let m = manifest.unwrap();
        let p = Path::new(&settings.fps_unlock_path)
            .follow_symlink()
            .unwrap()
            .to_path_buf();
        ...
        if enabled {
            download_or_update_fps_unlock(p, false);
        }
        Some(true)
    } else {
        None
    ...
    if install.is_some() {
        ...
        if fs::read_dir(pn.as_str()).unwrap().next().is_none() {
            std::thread::spawn(move || {
                ...
                .unwrap();
                let dm = get_compatibility(
                    archandle.as_ref(),
                    &runner_from_runner_version(dxvkv.as_str().to_string()).unwrap(),
                )
                .unwrap();
                let dv = dm
                    .versions
                    .into_iter()
                    .filter(|v| v.version.as_str() == dxvkv.as_str())
                    .collect::<Vec<_>>();
                let dxp = dv.get(0).unwrap().to_owned();
                let dxpp = Path::new(dxpp.as_str())
                    .follow_symlink()
                    .unwrap()
                    .to_path_buf();
                let rp = Path::new(runp.as_str())
                    .follow_symlink()
                    .unwrap()
                    .to_path_buf();
                ...
                if is_proton {
                    ...
                    if r0 {
                        ...
                        if r1.is_ok() {
                            Compat::add_dxvk(
                                winebin,
                                rpp.as_str().to_string(),
                                dxpp.to_str().unwrap().to_string(),
                                false,
                            )
                            .unwrap();
                            ...
                            send_notification(
                                &*archandle,
                                format!(
                                    "Download of {dxvn} complete.",
                                    dxvn = dxvkv.as_str().to_string()
                                )
                                .as_str(),
                                None,
                            );
...
pub fn game_launch(app: AppHandle, id: String) -> Option<bool> {
    ...
    if install.is_some() {
        ...
        if rslt {
            ...
            send_notification(
                &app,
                "Failed to launch game! Please check game.log file inside game directory for more information.",
                None,
            );
            None
        }
    ...
    #[cfg(target_os = "linux")]
    {
        match shortcut_type.as_str() {
            "desktop" => {
                ...
                let file = base
                    .join(format!("{}.desktop", install.name.as_str()))
                    .follow_symlink()
                    .unwrap();
...
"#,
                ...
                if status {
                    ...
                    send_notification(&app, format!("Failed to create {} desktop shortcut! If you use flatpak please make sure we have permission to access ~/.local/share/applications", install.name.as_str()).as_str(), None);
                }
            }
            "steam" => {
        ...
        match shortcut_type.as_str() {
            "desktop" => {
                ...
                if r.is_ok() {
                    ...
                    send_notification(&app, "Failed to create launch shortcut!", None);
                }
            }
            "steam" => {
                send_notification(
            ...
            "desktop" => {
                ...
                if status {
                    ...
                    send_notification(
                        &app,
                        format!(
                            "Desktop shortcut for {} does not exist!",
                            install.name.as_str()
                        )
                        .as_str(),
                        None,
            ...
            "desktop" => {
                ...
                if status {
                    ...
                    crate::utils::send_notification(
                        &app,
                        "Desktop shortcut for this game does not exist!",
                        None,
                    );
                }
            }
