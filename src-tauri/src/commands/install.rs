...
pub fn get_download_sizes(app: AppHandle, biz: String, version: String, lang: String, path: String, region: Option<String>) -> Option<String> {
    ...
    if manifest.is_some() {
        ...
        let gs = if m.biz == "bh3_global" { g.game.full.iter().filter(|e| e.region_code.clone().unwrap() == region.clone().unwrap_or("glb_official".to_string())).into_iter().map(|x| x.decompressed_size.parse::<u64>().unwrap()).sum::<u64>() } else { g.game.full.iter().map(|x| x.decompressed_size.parse::<u64>().unwrap()).sum::<u64>() };
        let mut fss = gs;
