...
use crate::utils::{AddInstallRsp, DownloadSizesRsp, ResumeStatesRsp, apply_xxmi_tweaks, copy_dir_all, extract_authkey_from_content, generate_cuid, get_mi_path_from_game, models::GameVersion, quote_shell_arg, resolve_shell_args_prefix, show_dialog};
...
use std::sync::Arc;
use crate::runtime::{LauncherContext, Emitter, Manager};
use sqlx::types::Json;
...
pub async fn list_installs(app: LauncherContext) -> Option<String> {
...
pub fn list_installs_by_manifest_id(app: LauncherContext, manifest_id: String) -> Option<String> {
...
pub fn set_installs_order(app: LauncherContext, order: Vec<(String, i32)>) {
    update_installs_order(&app, order);
}

pub fn get_install_by_id(app: LauncherContext, id: String) -> Option<String> {
...
pub fn add_install(app: LauncherContext, manifest_id: String, version: String, audio_lang: String, name: String, mut directory: String, mut runner_path: String, mut dxvk_path: String, runner_version: String, dxvk_version: String, game_icon: String, game_background: String, mut ignore_updates: bool, skip_hash_check: bool, mut use_jadeite: bool, use_xxmi: bool, use_fps_unlock: bool, env_vars: String, pre_launch_command: String, launch_command: String, fps_value: String, mut runner_prefix: String, launch_args: String, skip_game_dl: bool, region_code: String) -> Option<AddInstallRsp> {
...
pub async fn remove_install(app: LauncherContext, id: String, wipe_prefix: bool, keep_game_data: bool) -> Option<bool> {
    if id.is_empty() {
        None
    } else {
...
pub fn update_install_game_path(app: LauncherContext, id: String, path: String) -> Option<bool> {
...
pub fn update_install_runner_path(app: LauncherContext, id: String, path: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_dxvk_path(app: LauncherContext, id: String, path: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_skip_version_updates(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
...
pub fn update_install_skip_hash_valid(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
...
pub fn update_install_use_jadeite(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
...
pub fn update_install_use_xxmi(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
    let settings = get_settings(&app).unwrap();
...
pub fn update_install_use_fps_unlock(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
    let settings = get_settings(&app).unwrap();
...
pub fn update_install_fps_value(app: LauncherContext, id: String, fps: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
    let settings = get_settings(&app).unwrap();
...
pub fn update_install_graphics_api(app: LauncherContext, id: String, api: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
    if install.is_some() { let m = install.unwrap(); update_install_graphics_api_by_id(&app, m.id, api); Some(true) } else { None }
}

pub fn update_install_use_gamemode(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
...
pub fn update_install_use_mangohud(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
...
pub fn update_install_mangohud_config_path(app: LauncherContext, id: String, path: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_xxmi_config(app: LauncherContext, id: String, xxmi_hunting: Option<u64>, xxmi_sd: Option<bool>, xxmi_sw: Option<bool>, _engineini_tweaks: Option<bool>) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_show_drpc(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
...
pub fn update_install_disable_system_idle(app: LauncherContext, id: String, enabled: bool) -> Option<bool> {
    let manifest = get_install_info_by_id(&app, id);
...
pub fn update_install_env_vars(app: LauncherContext, id: String, env_vars: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_pre_launch_cmd(app: LauncherContext, id: String, cmd: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_launch_cmd(app: LauncherContext, id: String, cmd: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_game_background(app: LauncherContext, id: String, background: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
    if install.is_some() {
        let m = install.unwrap();
...
pub fn update_install_prefix_path(app: LauncherContext, id: String, path: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn update_install_launch_args(app: LauncherContext, id: String, args: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
#[cfg(target_os = "linux")]
pub fn update_install_runner_version(app: LauncherContext, id: String, version: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
#[cfg(target_os = "windows")]
pub fn update_install_runner_version(_app: LauncherContext, _id: String, _version: String) -> Option<bool> {
    None
}

#[cfg(target_os = "linux")]
pub fn update_install_dxvk_version(app: LauncherContext, id: String, version: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
#[cfg(target_os = "windows")]
pub fn update_install_dxvk_version(_app: LauncherContext, _id: String, _version: String) -> Option<bool> {
    None
}

pub fn game_launch(app: LauncherContext, id: String) -> Option<bool> {
    let install = get_install_info_by_id(&app, id);
...
pub fn check_game_running(app: LauncherContext, id: String) -> Option<String> {
    let install = get_install_info_by_id(&app, id.clone());
...
pub fn get_download_sizes(app: LauncherContext, biz: String, version: String, lang: String, path: String, region: Option<String>) -> Option<String> {
...
pub fn get_resume_states(app: LauncherContext, install: String) -> Option<String> {
...
pub fn add_shortcut(app: LauncherContext, install_id: String, shortcut_type: String) {
    ...
    #[cfg(target_os = "linux")]
    {
        match shortcut_type.as_str() {
            "desktop" => {
                ...
                let bin_name = std::env::var("TTL_SHELL_COMMAND").ok().filter(|v| !v.is_empty()).unwrap_or_else(|| if is_flatpak() { "flatpak run app.twintaillauncher.ttl".to_string() } else { "twintaillauncher".to_string() });
...
"#,
            ...
            "steam" => {
                ...
                let shell_exe = std::env::var("TTL_SHELL_EXECUTABLE").ok().filter(|v| !v.is_empty()).unwrap_or_else(|| if is_flatpak() { "flatpak".to_string() } else { "twintaillauncher".to_string() });
                let mut launchargs_parts = if is_flatpak() && std::env::var("TTL_SHELL_ARGS_PREFIX").is_err() { vec!["run".to_string(), "app.twintaillauncher.ttl".to_string()] } else { resolve_shell_args_prefix() };
                launchargs_parts.push(format!("--install={}", install.id.as_str()));
                let launchargs = launchargs_parts.iter().map(|part| { quote_shell_arg(part.as_str()) }).collect::<Vec<_>>().join(" ");

                let shortcut = Shortcut {
                    order: "",
                    app_id: calculate_app_id(m.paths.exe_filename.as_str(), install.name.as_str()),
                    app_name: install.name.as_str(),
                    exe: shell_exe.as_str(),
    ...
    #[cfg(target_os = "windows")]
    {
        match shortcut_type.as_str() {
            "desktop" => {
                ...
                let bin_name = std::env::var("TTL_SHELL_EXECUTABLE").ok().filter(|v| !v.is_empty()).map(PathBuf::from).unwrap_or_else(|| { app.path().app_local_data_dir().unwrap().join("twintaillauncher.exe") });
                let file = base.join(format!("{}.lnk", install.name.as_str()));
                let mut shortcut_args = resolve_shell_args_prefix();
                shortcut_args.push(format!("--install={}", install.id.as_str()));
                let launch_args = shortcut_args.iter().map(|arg| { quote_shell_arg(arg.as_str()) }).collect::<Vec<_>>().join(" ");
                let sl = shortcuts_rs::ShellLink::new(bin_name.as_path(), Some(launch_args), Some(install.name.clone()), None).unwrap();
...
pub fn remove_shortcut(app: LauncherContext, install_id: String, shortcut_type: String) {
    let install = get_install_info_by_id(&app, install_id).unwrap();
    #[cfg(target_os = "linux")]
    {
        match shortcut_type.as_str() {
...
pub fn copy_authkey(app: LauncherContext, id: String) -> bool {
...
fn enqueue_extras_download(app: &LauncherContext, path: String, package_id: String, package_type: String, update_mode: bool) {
    let state = app.state::<DownloadState>();
    let q = state.queue.lock().unwrap().clone();
...
pub fn cancel_download_for_install(app: &LauncherContext, install_id: &str) {
    let state = app.state::<DownloadState>();
