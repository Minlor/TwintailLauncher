...
use std::sync::{Arc};
use std::{fs, io};
use crate::runtime::{LauncherContext, Emitter, Listener, Manager};
...
pub fn resolve_app_data_dir(app: &LauncherContext) -> PathBuf {
    if let Ok(dir) = std::env::var("TTL_DATA_DIR") { if !dir.is_empty() { return PathBuf::from(dir); } }
    app.path().app_data_dir().unwrap()
}

pub fn resolve_resource_dir(app: &LauncherContext) -> PathBuf {
    if let Ok(dir) = std::env::var("TTL_RESOURCE_DIR") { if !dir.is_empty() { return PathBuf::from(dir); } }
    app.path().resource_dir().unwrap()
}

pub fn quote_shell_arg(arg: &str) -> String {
    if arg.is_empty() { return "\"\"".to_string(); }
    if !arg.chars().any(|c| c.is_whitespace() || c == '"' || c == '\\') { return arg.to_string(); }
    format!("\"{}\"", arg.replace('\\', "\\\\").replace('"', "\\\""))
}

pub fn resolve_shell_args_prefix() -> Vec<String> {
    if let Ok(value) = std::env::var("TTL_SHELL_ARGS_PREFIX") { return serde_json::from_str::<Vec<String>>(&value).unwrap_or_default(); }
    Vec::new()
}

pub fn run_async_command<F: Future>(cmd: F) -> F::Output {
    if tokio::runtime::Handle::try_current().is_ok() { tokio::task::block_in_place(|| tokio::runtime::Handle::current().block_on(cmd)) } else { tokio::runtime::Runtime::new().unwrap().block_on(cmd) }
}

pub fn copy_dir_all(app: &LauncherContext, src: impl AsRef<Path>, dst: impl AsRef<Path>, install: String, install_name: String, install_type: String) -> io::Result<()> {
...
pub fn register_listeners(app: &LauncherContext) {
    let h1 = app.clone();
    app.listen("launcher_action_exit", move |_event| {
        if let Some(window) = h1.get_window("main") { let _ = window.hide(); }
        h1.cleanup_before_exit();
        h1.exit(0);
        std::process::exit(0);
    });
    let h2 = app.clone();
    app.listen("launcher_action_minimize", move |_event| { if let Some(window) = h2.get_window("main") { let _ = window.minimize(); } });
...
pub fn show_dialog(app: &LauncherContext, dialog_type: &str, title: &str, message: &str, buttons: Option<Vec<&str>>) {
    show_dialog_with_callback(app, dialog_type, title, message, buttons, None);
}

pub fn show_dialog_with_callback(app: &LauncherContext, dialog_type: &str, title: &str, message: &str, buttons: Option<Vec<&str>>, callback_id: Option<&str>) {
...
pub fn setup_or_fix_default_paths(app: &LauncherContext, path: PathBuf, fix_mode: bool) {
...
#[cfg(target_os = "linux")]
pub fn sync_installed_runners(app: &LauncherContext) {
    let gs = get_settings(app);
    if gs.is_some() {
        let s = gs.unwrap();
...
pub fn sync_install_backgrounds(app: &LauncherContext) {
...
#[cfg(target_os = "linux")]
pub fn deprecate_jadeite(app: &LauncherContext) {
    let installs = get_installs(app);
    if installs.is_some() {
...
#[cfg(target_os = "linux")]
pub fn fix_window_decorations(app: &LauncherContext) {
...
#[cfg(target_os = "linux")]
pub fn apply_patch(app: &LauncherContext, dir: String, patch_type: String, mode: String) {
    ...
    if dir.exists() {
        match patch_type.as_str() {
            ...
            "sparkle" => {
                ...
                match mode.as_str() {
                    "add" => {
                        let patch = resolve_resource_dir(app).join("resources").join("hkrpg_patch.dll");
