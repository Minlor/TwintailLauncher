...
use crate::runtime::{LauncherContext, Emitter, Error};
...
#[cfg(target_os = "linux")]
pub fn launch(app: &LauncherContext, install: LauncherInstall, gm: GameManifest, gs: GlobalSettings) -> Result<bool, Error> {
...
#[cfg(target_os = "linux")]
fn load_xxmi(app: &LauncherContext, install: LauncherInstall, prefix: String, xxmi_path: String, runner: String, wine64: String, game: String, is_proton: bool) {
    if install.use_xxmi {
        let appc = app.clone();
...
#[cfg(target_os = "linux")]
fn load_fps_unlock(app: &LauncherContext, install: LauncherInstall, biz: String, prefix: String, fpsunlock_path: String, game_path: String, runner: String, wine64: String, is_proton: bool) {
...
#[cfg(target_os = "linux")]
fn run_winetricks(app: &LauncherContext, install: LauncherInstall, steamrt: String, reaper: String, appid: u32, runner: String, wine64: String, prefix: String, install_dir: String, verbs: Vec<String>) -> std::thread::JoinHandle<bool> {
    let appc = app.clone();
    // Prevent "App is not responding" by waiting in a separate thread
    std::thread::spawn(move || {
        let app = appc.clone();
...
#[cfg(target_os = "windows")]
fn load_xxmi(app: &LauncherContext, install: LauncherInstall, xxmi_path: String, game: String) {
    if install.use_xxmi {
...
#[cfg(target_os = "windows")]
fn load_fps_unlock(app: &LauncherContext, install: LauncherInstall, biz: String, game_path: String, fpsunlock_path: String) {
    if install.use_fps_unlock {
...
fn start_playtime_tracker(app: &LauncherContext, install: LauncherInstall, gm: GameManifest, exe_name: String) {
    let app = app.clone();
...
fn write_log(app: &LauncherContext, log_dir: PathBuf, child: Child, file: String) {
