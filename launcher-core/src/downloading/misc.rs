...
#[cfg(target_os = "linux")]
pub fn download_or_update_steamrt3(app: &LauncherContext) {
...
#[cfg(target_os = "linux")]
pub fn run_steamrt3_download(app: LauncherContext, payload: SteamrtDownloadPayload, job_id: String) -> QueueJobOutcome {
...
#[cfg(target_os = "linux")]
pub fn download_or_update_steamrt4(app: &LauncherContext) {
    let gs = get_settings(app);
    if let Some(s) = gs {
        let rp = Path::new(&s.default_runner_path);
...
#[cfg(target_os = "linux")]
pub fn run_steamrt4_download(app: LauncherContext, payload: SteamrtDownloadPayload, job_id: String) -> QueueJobOutcome {
    let job_id = Arc::new(job_id);
    let dlpayload: Arc<Mutex<HashMap<String, String>>> = Arc::new(Mutex::new(HashMap::new()));
    let steamrt_path = PathBuf::from(&payload.steamrt_path);
    let event_name = if payload.is_update { "update_progress" } else { "download_progress" };
...
#[cfg(target_os = "linux")]
pub fn run_runner_download(app: LauncherContext, payload: RunnerDownloadPayload, job_id: String) -> QueueJobOutcome {
    let job_id = Arc::new(job_id);
    let dlpayload: Arc<Mutex<HashMap<String, String>>> = Arc::new(Mutex::new(HashMap::new()));
...
pub fn check_extras_update(app: &LauncherContext) {
    let gs = get_settings(app);
    if gs.is_some() {
        let s = gs.unwrap();
...
pub fn download_or_update_extra(app: &LauncherContext, path: PathBuf, package_id: String, package_type: String, update_mode: bool, job_id: Option<String>) -> bool {
    if job_id.is_none() {
        if !update_mode {
            let state = app.state::<DownloadState>();
            let q = state.queue.lock().unwrap().clone();
